...
/*        if($config['logout_url']){
    ...
    /**
    * Funcao para adicionar mascara a um inteiro
   */
    ...
    public function _getAuthenticatedUser() {
        ...
        if($this->_validateResponse()){
            ...
            $auth_cpf = $response['auth']['raw']['preferred_username'];

            $cpf = (isset($auth_cpf)) ? $this->mask($auth_cpf,'###.###.###-##') : null;

            if (!empty($cpf)) {
                $agent_meta = $app->repo('AgentMeta')->findOneBy(["key" => 'cpf', "value" => $cpf]);
...
//            $user = $app->repo('User')->getByAuth($auth_provider, $auth_uid);
    ...
    protected function _createUser($response) {
        ...
        if(isset($response['auth']['raw']['name'])){
            ...
            $agent->cpf = $response['auth']['raw']['preferred_username'];
//            $agent->dataDeNascimento = $response['auth']['raw']['dataNascimento'];
        }else{
            $agent->name = '';
        }
